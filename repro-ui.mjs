...
function formatLatexToHtml(input) {
  ...
  while (true) {
    ...
    if (depth === 0) {
      ...
      const html = `[[BOX_START]]${formatLatexToHtml(inner)}[[BOX_END]]`;
...
// bank-gen conversion (simulated)
const afterBankGen = `Ta chứng minh "$\\forall n \\in \\mathbb{N}\\, P(n)$" bằng quy nạp.`;

console.log("UI Result:", formatLatexToHtml(afterBankGen));

const nestedInput = `\\dongkhung{Ta chứng minh "$\\forall n \\in \\mathbb{N}\\, P(n)$" bằng quy nạp.}`;
console.log("UI Nested Result:", formatLatexToHtml(nestedInput));
