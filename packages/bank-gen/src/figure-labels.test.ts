...
describe("replaceFigureReferences", () => {
    ...
    it("should use 'Figure' for 'en' language", () => {
        ...
        expect(output).toBe('See <a href="#fig-fig:test" class="latex-ref">Figure [[FIG_NUM_fig:test]]</a>');
    });

    it("should use 'Hình' for 'vi' language", () => {
        const input = "See \\figurename~\\ref{fig:test}";
        const output = replaceFigureReferences(input, labelMap, "vi");
        expect(output).toBe('See <a href="#fig-fig:test" class="latex-ref">Hình [[FIG_NUM_fig:test]]</a>');
    });
    ...
    it("should ignore LaTeX package detection (implicit logic removed)", () => {
        ...
        const output = replaceFigureReferences(input, labelMap, "vi");
        expect(output).toBe('\\usepackage[english]{babel} See <a href="#fig-fig:test" class="latex-ref">Hình [[FIG_NUM_fig:test]]</a>');
    });
    ...
    it("should ignore LaTeX package detection for english", () => {
        ...
        const output = replaceFigureReferences(input, labelMap, "en");
        expect(output).toBe('\\usepackage{vietnam} See <a href="#fig-fig:test" class="latex-ref">Figure [[FIG_NUM_fig:test]]</a>');
    });
});
