...
export function ExamPage({ session, setSession }) {
    ...
    useEffect(() => {
        if (!confirmOpen && !clearConfirmOpen)
            return;
        const onKeyDown = (e) => {
            if (e.key === "Escape") {
    ...
    const layout = (_jsxs(PageShell, { className: "py-0 pb-24 lg:pb-10", children: [_jsxs(Card, { className: "mt-4 space-y-3", children: [_jsxs("div", { children: [_jsxs("h1", { className: "text-lg font-semibold text-text", children: ["Exam ", config?.examId ?? examId, config?.subject ? ` — ${config.subject}` : ""] }), _jsx("p", { className: "text-sm text-textMuted", children: "Answer questions and submit when done." })] }), timeLimitMinutes ? (_jsxs("div", { className: "flex flex-wrap items-center gap-2 text-sm", children: [_jsx(Badge, { tone: timeExpired ? "warn" : "info", children: timeRemainingMs === null ? "Time limit" : formatTimeRemaining(timeRemainingMs) }), _jsxs("span", { className: "text-xs text-textMuted", children: ["Limit: ", timeLimitMinutes, " minutes"] })] })) : null, _jsx(StepIndicator, { steps: startSteps }), _jsx("div", { role: "status", "aria-live": "polite", className: "text-xs text-textMuted min-h-[1.25rem]", children: draftNotice ?? "" })]) }), submission ? (_jsxs(Card, { className: "mt-4 space-y-2", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("div", { className: "text-sm font-semibold text-text", children: "Score" }), _jsxs(Badge, { tone: "success", children: [submission.score.correct, "/", submission.score.total] })] }), _jsx("div", { className: "text-sm text-textMuted", children: "Submission recorded." }), signedIn ? (_jsx(Link, { to: "/history", className: "inline-flex", children: _jsx(Button, { variant: "secondary", size: "sm", children: "View in My History" }) })) : null] })) : null, status ? _jsx(Alert, { tone: status.tone, className: "mt-3", children: status.text }) : null, _jsxs("div", { className: "grid gap-4 lg:grid-cols-[2fr,1fr] mt-4", children: [_jsx("div", { className: "space-y-4", children: bank ? (bank.questions.map((q, idx) => (_jsx("div", { id: `q-${idx + 1}`, children: _jsx(McqQuestion, { index: idx, question: q, answer: answers[q.uid], onChange: (uid, val) => setAnswers((prev) => ({ ...prev, [uid]: val })), showSolution: showSolutions && config?.policy.solutionsMode !== "never", submissionStatus: submission?.perQuestion?.find((p) => p.uid === q.uid)?.correct === true
                                    ...
                                    : submission
                                        ...
                                        }) })] }), _jsxs(Accordion, { title: "Authentication", defaultOpen: true, tone: "muted", children: [_jsxs("div", { className: "flex gap-2 flex-wrap", children: [_jsx(Button, { variant: "secondary", size: "sm", onClick: handleGithub, children: "GitHub Login" }), _jsx(Button, { variant: "secondary", size: "sm", onClick: handleGoogle, children: "Google Login" }), config?.policy.authMode === "optional" ? (_jsx(Button, { variant: "ghost", size: "sm", onClick: handleAnonymous, children: "Continue anonymously" })) : null] }), _jsx("div", { className: "text-xs text-textMuted", children: "Signing in lets you view submission history and helps keep a consistent per-student version on repeat visits." }), _jsxs("div", { className: "text-xs text-textMuted", children: ["Current session: ", session ? session.provider : "none"] })] }), _jsxs(Accordion, { title: "Access codes", defaultOpen: true, tone: requireViewCode || requireSubmitCode ? "warn" : "muted", children: [_jsx("div", { className: "text-xs text-textMuted", children: "View code unlocks questions. Submit code is only required when you submit your final answers." }), requireViewCode ? (_jsxs("div", { className: "space-y-1", children: [_jsx("label", { className: "text-sm font-medium", children: "View code" }), _jsx("input", { className: "w-full rounded-lg border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-info", value: viewCode, onChange: (e) => setViewCode(e.target.value), placeholder: "Enter view code" })] })) : (_jsx("div", { className: "text-sm text-textMuted", children: "No view code required." })), _jsx(Button, { variant: "primary", size: "sm", onClick: handleBank, children: "Load questions" }), requireSubmitCode ? (_jsxs("div", { className: "space-y-1", children: [_jsx("label", { className: "text-sm font-medium", children: "Submit code" }), _jsx("input", { className: "w-full rounded-lg border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-info", value: submitCode, onChange: (e) => setSubmitCode(e.target.value), placeholder: "Enter submit code" })] })) : (_jsx("div", { className: "text-sm text-textMuted", children: "No submit code required." }))] }), _jsxs(Card, { className: "space-y-3", children: [_jsxs("div", { className: "flex justify-between items-center", children: [_jsx("div", { className: "font-semibold text-sm", children: "Submit" }), _jsxs(Badge, { tone: submitDisabled ? "warn" : "info", children: [answeredCount, "/", totalQuestions, " answered"] })] }), _jsx(Button, { variant: "secondary", onClick: handleSaveForLater, disabled: !bank || !versionId, className: "w-full", children: "Save & submit later" }), _jsx(Button, { onClick: openSubmitConfirm, disabled: submitDisabled, className: "w-full", children: "Submit answers" })] })] })] })] }));
    if (loading) {
