...
let cachedTimeFormat: string | null = null;
...
export async function initDefaultTimezone(fetcher: () => Promise<string | null>): Promise<void> {
  try {
    ...
    if (tz) setCachedTimezone(tz);
  } catch {
    // best-effort only
  }
}

export function getCachedTimeFormat(): string | null {
...
export function setCachedTimeFormat(format: string | null): void {
...
export async function initDefaultTimeFormat(fetcher: () => Promise<string | null>): Promise<void> {
  try {
    const fmt = await fetcher();
    if (fmt) setCachedTimeFormat(fmt);
    else setCachedTimeFormat(DEFAULT_TIME_FORMAT);
  } catch {
    // best-effort only
  }
}
...
export function listTimeFormats(): TimeFormatOption[] {
  return TIME_FORMAT_OPTIONS;
}

export function getTimeFormatLabel(id: string) {
  return TIME_FORMAT_OPTIONS.find((option) => option.id === id)?.label ?? TIME_FORMAT_OPTIONS[0].label;
}

export function getTimeFormatById(id?: string) {
  return TIME_FORMAT_OPTIONS.find((option) => option.id === id) ?? TIME_FORMAT_OPTIONS[0];
}

function formatWithOption(formatId: string, date: Date, timeZone?: string | null) {
...
export function onTimeFormatChange(handler: () => void): () => void {
  if (typeof window === "undefined") return () => {};
  const listener = () => handler();
