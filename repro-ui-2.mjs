...
function formatLatexToHtml(input) {
    ...
    while (true) {
        ...
        if (start === -1) break;
        let braceStart = text.indexOf("{", start); // Updated to find { anywhere after \dongkhung
        braceStart = text.indexOf("{", start + 10);
        ...
        if (depth === 0) {
            ...
            const html = formatLatexToHtml(inner); // Recursive
    ...
    // DEBUG
    // console.log("Text before replacement:", result);

    blocks.forEach((html, i) => {
        const token = `${tokenPrefix}${i}__`;
        // console.log("Replacing", token, "with", html);
...
console.log("Result:", formatLatexToHtml(nestedInput));
